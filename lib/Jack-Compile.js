...
function run_f5() {
    run(atom.config.get("Jack-Compile.f5_command"));
}

function run_f6() {
    run(atom.config.get("Jack-Compile.f6_command"));
}
...
function run(command) {
    ...
    if (info.ext != ".jack") {
        notification("warning", format("{0} is not a .jack file, pay Richard 1$, exit.", [info.base]));
        return;
    }
